...
import re_sch_oline_getContactName from '@salesforce/apex/CalledForInteractionOnlineController_LWC.re_sch_oline_getContactName';
...
import reSch_oline_createEvent from '@salesforce/apex/CalledForInteractionOnlineController_LWC.reSch_oline_createEvent';
import { CurrentPageReference } from 'lightning/navigation';

export default class CalledForInteractionOnlineReschedule_LWC extends NavigationMixin(LightningElement) {
    ...
    fetchContactName() { 
        re_sch_oline_getContactName({ AdmissioFormId: this.recordId })
            .then(result => {
                ...
                if (!result.interactionSchedule) {
                    this.showToast('Error', 'Please Schedule Interaction Online First', 'error');
                    this.interactionSchedule = result.interactionSchedule;
                    console.error('this.interactionSchedule', this.interactionSchedule);
                } else
                if (result.interactionre_Schedule) {
                    this.showToast('Error', 'Interaction Online is already re-scheduled', 'error');
                    this.interactionreSchedule = result.interactionre_Schedule;
    ...
    handleSubmit() {
        ...
        reSch_oline_createEvent({
            ...
            adminssionId: this.recordId,
            school: this.schoolid,
            interactionSchedule : this.interactionSchedule,
            interactionre_Schedule : this.interactionreSchedule
        })
            .then(result => {
